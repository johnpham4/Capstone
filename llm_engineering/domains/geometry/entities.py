...
class Diagram:
    ...
    # [NEW] Danh sách chứa các tứ giác (hình vuông) để vẽ nét liền
    ...
    def add_quadrilateral(self, points: List[GeometricPoint]) -> None:
        """[NEW] Add a quadrilateral (square) to the diagram"""
        if len(points) != 4:
            raise ValueError("Quadrilateral must have exactly 4 points")
        self.quadrilaterals.append(points)
