...
app = FastAPI(
    ...
    description="Simple API for geometry diagram generation",
    version="1.0.0"
)
...
class DiagramResponse(BaseModel):
    """Response containing generated diagram DSL"""
    request_id: str
    user_input: str
    model_output: str
    status: str
...
class RenderRequest(BaseModel):
    """Request to render a diagram from DSL"""
    dsl: str = Field(..., description="Geometry DSL code (S-expression syntax)")
    title: Optional[str] = Field(None, description="Optional title for the diagram")
    dpi: Optional[int] = Field(150, description="Image resolution (DPI)")
    epochs: Optional[int] = Field(2000, description="Optimization epochs")
    n_tries: Optional[int] = Field(1, description="Number of optimization attempts")
...
@app.post("/api/v1/diagrams/generate-and-render")
