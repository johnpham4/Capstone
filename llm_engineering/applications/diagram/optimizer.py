...
class Optimizer:
    ...
    def get_diagram(self):
        ...
        if hasattr(diagram, 'add_quadrilateral'):
             for quad_names in self.quadrilaterals:
                points = []
                valid = True
                for name in quad_names:
                    if name in diagram.points:
                        points.append(diagram.points[name])
                    else:
                        valid = False
                if valid:
                    diagram.add_quadrilateral(points)
